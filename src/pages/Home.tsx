        ...
        <div className="lg:col-span-5 h-full relative bg-brand-sand/50 min-h-[400px]">
          <div className="absolute inset-0 flex items-center justify-center p-8">
            <div className="relative w-full max-w-sm aspect-[4/5] bg-white rounded-2xl shadow-2xl overflow-hidden border-8 border-white transform rotate-3">
              <img
                src="/src/assets/images/regenerated_image_1778091014698.jpg"
                alt="Combo Pack"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 bg-white">
            {featuredProducts.map((product) => (
              <motion.div
                key={product.id}
                whileHover={{ y: -10 }}
                className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-2xl transition-all border border-stone-100 flex flex-col h-full"
              >
                <Link to={`/product/${product.id}`} className="block relative aspect-[4/3] overflow-hidden">
                  <img
                    ...
                    referrerPolicy="no-referrer"
                  />
                  <div className="absolute bottom-4 left-4 bg-white/90 backdrop-blur-md px-3 py-1 rounded text-xs font-bold tracking-widest text-brand-dark uppercase">
                    {product.category}
                  </div>
                </Link>
                <div className="p-8 flex flex-col flex-1">
                  <div className="flex justify-between items-start mb-4">
                    <h3 className="text-2xl font-black font-bn tracking-tight">
                      {product.nameBn}
                    </h3>
